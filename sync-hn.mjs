 ...
 *
 * Usage: node sync-hn.js [count] [--loop=hours]
 */
...
async function minimaxChat(prompt, systemPrompt = '') {
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (i < 2) {
        console.log('  [MiniMax retry] ' + err.message + ', attempt ' + (i + 1) + '/3');
...
async function translateToChinese(text) {
  const result = await minimaxChat(
    ...
    'You are a professional translator. Output only the Chinese translation, keep full meaning.'
  );
  if (!result) return text;
  // Extract all Chinese chars, join into complete sentence
  const chinese = result.match(/[\u4e00-\u9fff]+/g);
  if (chinese && chinese.length > 0) {
    return chinese.join('');
  }
  return result.replace(/[^\u4e00-\u9fff]+/g, '').substring(0, 50);
}
...
async function upsertBlogPost(story, titleEn, titleZh, contentEn, contentZh) {
  if (!contentEn || contentEn.length < 200) {
    console.log('  [SKIP] content_en too short (' + (contentEn ? contentEn.length : 0) + '), need 300+');
    return;
  }
  if (!contentZh || contentZh.length < 200) {
    console.log('  [SKIP] content_zh too short (' + (contentZh ? contentZh.length : 0) + '), need 300+');
    return;
  }
...
async function main() {
  ...
  const loopHours = parseFloat(args.find(a => a.startsWith('--loop=')) ? args.find(a => a.startsWith('--loop=')).split('=')[1] : '0');
